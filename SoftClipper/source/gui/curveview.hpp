...
public:
  SomeDSP::SoftClipper<float> clipper;
  ...
  void draw(CDrawContext *pContext) override
  {
    ...
    auto &&clip = 2.0f * clipper.clipY; // [0, clip] is the range of horizontal axis.
    ...
    for (size_t idx = 0; idx < points.size(); ++idx) {
      auto &&value = clipper.process(clip * float(idx) / (float(width) - 1.0f));
      points[idx].x = idx;
      points[idx].y = ampToHeight(clip, value);
    }
    ...
    auto &&clipStartAmp = clipper.ratio * clipper.clipY;
    auto &&outPeakAmp = clipper.process(holdValue);
    std::ostringstream os;
    os.precision(5);
    os << std::fixed                                                                 //
       << "Clip Start" << cdlm << ampToDecibel(clipStartAmp) << cdlm << "dB" << rdlm //
       << "Clip End" << cdlm << ampToDecibel(clipper.clipY) << cdlm << "dB" << rdlm  //
       << "Input Peak" << cdlm << ampToDecibel(holdValue) << cdlm << "dB" << rdlm    //
       << "Output Peak" << cdlm << ampToDecibel(outPeakAmp) << cdlm << "dB" << rdlm;
