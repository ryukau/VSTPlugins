...
{
  auto smoothness = param.value[ParameterID::smoothness]->getRaw();
  ...
  Vst::ParamValue time = param.value[ParameterID::time]->getRaw();
  if (param.value[ParameterID::tempoSync]->getRaw() > 0.5) {
    if (time < 1.0)
      time *= 15.0 / tempo;
    else
  ...
  Vst::ParamValue offset = param.value[ParameterID::offset]->getRaw();
  if (offset < 0.0) {
  ...
  interpFeedback.push(
    param.value[ParameterID::negativeFeedback]->getRaw() > 0.5
  ...
  const size_t length, float *in0, float *in1, float *out0, float *out1)
{
  for (size_t i = 0; i < length; ++i) {
    ...
    if (param.value[ParameterID::lfoHold]->getRaw() < 0.5) {
