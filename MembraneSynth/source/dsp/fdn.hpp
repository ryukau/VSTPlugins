...
public:
  ...
  void noteOn(Sample amplitude, Sample decayTimeSamples)
  {
    decay = std::pow(epsilon, Sample(1) / decayTimeSamples);
    gain = amplitude / decay;
  }

  void reset() { gain = 0; }

  Sample process() { return gain *= decay; }
};
...
template<typename Sample, size_t length> class ModulatedFDN {
private:
