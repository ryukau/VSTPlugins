...
DrawBlock::DrawBlock(Context &context) : context(context)
{
  ...
  if (clip.isEmpty()) {
    ...
    cairo_rectangle(
      context.getCairo(), clip.left, clip.top, clip.getWidth(), clip.getHeight());
...
//------------------------------------------------------------------------
DrawBlock DrawBlock::begin(Context &context) { return DrawBlock(context); }

//-----------------------------------------------------------------------------
Context::Context(const CRect &rect, const SurfaceHandle &surface)
  : super(rect), surface(surface)
{
  init();
}
...
Context::Context(Bitmap *bitmap)
  : super(new CBitmap(bitmap)), surface(bitmap->getSurface())
{
  init();
}
...
void Context::init()
{
  if (surface) cr.assign(cairo_create(surface));
  super::init();
}

//-----------------------------------------------------------------------------
CRect Context::getCurrentStateClipRect() const { return getCurrentState().clipRect; }
...
{
  cairo_restore(cr);
  if (surface) cairo_surface_flush(surface);
  checkCairoStatus(cr);
  super::endDraw();
}

//-----------------------------------------------------------------------------
void Context::saveGlobalState() { super::saveGlobalState(); }

//-----------------------------------------------------------------------------
void Context::restoreGlobalState() { super::restoreGlobalState(); }
...
void Context::setSourceColor(CColor color)
{
  ...
  cairo_set_source_rgba(
    cr, color.normRed<double>(), color.normGreen<double>(), color.normBlue<double>(),
    alpha);
  checkCairoStatus(cr);
}
...
void Context::setupCurrentStroke()
{
  ...
  if (!style.getDashLengths().empty()) {
    ...
    for (auto &l : lengths) l *= lineWidth;
...
//-----------------------------------------------------------------------------
void Context::drawPolygon(
  const CDrawContext::PointList &polygonPointList, const CDrawStyle drawStyle)
{
  if (polygonPointList.size() < 2) return;
...
//-----------------------------------------------------------------------------
void Context::drawBitmap(
  CBitmap *bitmap, const CRect &dest, const CPoint &offset, float alpha)
{
  if (auto cd = DrawBlock::begin(*this)) {
    double transformedScaleFactor = getScaleFactor();
    CGraphicsTransform t = getCurrentTransform();
    if (t.m11 == t.m22 && t.m12 == 0 && t.m21 == 0) transformedScaleFactor *= t.m11;
    auto cairoBitmap = bitmap->getBestPlatformBitmapForScaleFactor(transformedScaleFactor)
                         .cast<Bitmap>();
    if (cairoBitmap) {
      ...
      cairo_matrix_init_scale(
        &matrix, cairoBitmap->getScaleFactor(), cairoBitmap->getScaleFactor());
      cairo_matrix_translate(&matrix, offset.x, offset.y);
      cairo_pattern_set_matrix(pattern, &matrix);
      cairo_set_source(cr, pattern);

      cairo_rectangle(
        cr, -offset.x, -offset.y, dest.getWidth() + offset.x,
...
//-----------------------------------------------------------------------------
void Context::fillLinearGradient(
  CGraphicsPath *path,
  ...
  CGraphicsTransform *transformation)
{
  if (path) {
    ...
    if (
      auto cairoGradient
      = dynamic_cast<Gradient *>(gradient.getPlatformGradient().get())) {
      if (auto cd = DrawBlock::begin(*this)) {
        auto p = alignedPath ? alignedPath->getCairoPath() : graphicsPath->getCairoPath();
        cairo_append_path(cr, p);
...
//-----------------------------------------------------------------------------
void Context::fillRadialGradient(
  CGraphicsPath *path,
  const CGradient &gradient,
  const CPoint &center,
  CCoord radius,
